...
// Le domaine d'une personne, c'est son équipe : la colonne `Team.role`.
function domainesConnus() {
    const vus = [];
    for (const m of team) if (m.role && !vus.includes(m.role)) vus.push(m.role);
    return vus.sort();
}
...
function updateFilterMenus() {
    ...
    const domHtml = domaines.map((d, rang) => `<div class="filter-option" onclick="basculerDomaine(${rang})"><input type="checkbox" ${filters.domaine.includes(d)?'checked':''}>${escapeHtml(d)}</div>`).join('');
    ...
    if (menu) menu.innerHTML = grp('Projets', projHtml, true) + grp('Priorités', prioHtml) + grp('Assignés', assHtml) + (domaines.length ? grp('Domaines', domHtml) : '') + (total ? '<div class="fm-foot"><span class="fm-clear" onclick="clearAllGanttFilters()">Tout effacer</span></div>' : '');
    updateFilterUI();
}
...
const personnesDeLigne = (t) => (t && t.Responsable ? [t.Responsable] : []).concat(getAssigneesArray(t));

// Une ligne appartient à tous les domaines qui la touchent : les siens, ceux de son chantier, celui
// du responsable de son projet. Un directeur de domaine veut voir ce qui occupe son équipe, pas
// seulement ce qu'elle pilote.
function domainesDeLigne(t) {
    const ids = personnesDeLigne(t);
    if (!estChantier(t) && t.chantier) {
        const c = tasks.find(x => estChantier(x) && x.idChantier === t.chantier);
        if (c) ids.push(...personnesDeLigne(c));
    }
    const projet = projects.find(p => p.id === t.projet);
    if (projet && projet.responsable) ids.push(projet.responsable);
    const domaines = [];
    for (const id of ids) {
        const m = team.find(x => x.id === id);
        if (m && m.role && !domaines.includes(m.role)) domaines.push(m.role);
    }
    return domaines;
}
...
function clearAllGanttFilters() { filters = { project: [], assignee: [], priority: [], domaine: [] }; updateFilterUI(); broadcastFilters(); persistFilters(); render(); }
