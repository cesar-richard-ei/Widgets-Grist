...
async function fusionnerChantiers() {
    const colonne = colonneChantier();
    if (!colonne) return;
    if (!tableDeclaree('Chantiers')) return;
    let brut;
...
async function loadAllData(prefetched) {
    ...
    categoriesProjet = [];
    if (tableDeclaree('Categorie_de_projet')) {
        try { categoriesProjet = convert(await grist.docApi.fetchTable('Categorie_de_projet')); } catch (e) { noterRefus('Categorie_de_projet'); }
    }
    gristReady = true;
