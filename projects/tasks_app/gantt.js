...
function updateFilterMenus() {
    ...
    const domHtml = domaines.map((d, rang) => `<div class="filter-option" data-filtre="domaine" data-valeur="${escapeHtml(d)}" onclick="basculerDomaine(${rang})"><input type="checkbox" ${filters.domaine.includes(d)?'checked':''}><span class="dot" style="background:${couleurDeDomaine(d)}"></span>${escapeHtml(d)}</div>`).join('');
