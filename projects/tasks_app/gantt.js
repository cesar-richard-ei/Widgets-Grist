...
// Les tâches sont lues après le premier rendu : tant qu'elles manquent, la plage dessinée n'est pas
// celle du document, et le calage posé dessus ne vaut pas.
let tachesLues = false;
...
// textContent puis innerHTML ne touche pas aux guillemets : une valeur du document placée dans un
// attribut peut en sortir et poser son propre gestionnaire d'événement.
const escapeAttr = (t) => escapeHtml(t).replace(/"/g, '&quot;');
// Les couleurs viennent aussi du document, et partent dans un attribut style.
...
function fusionnerChantiers(brut) {
    const colonne = colonneChantier();
    if (!colonne || !brut) return;
...
function sectionChantier(data, recherche, sansResultat) {
    if (!colonneChantier()) return '';
    const chantiers = tasks.filter(estChantier);
    if (!chantiers.length) return '';
    // Ordre alphabétique et champ de recherche, comme les autres sélecteurs du volet : la liste
    // suivait l'ordre d'affichage du Gantt, où retrouver un chantier parmi des dizaines se fait à
    // l'oeil.
    const tries = chantiers.slice().sort((a, b) => (a.titre || '').localeCompare(b.titre || '', 'fr'));
    const options = tries.map(c => '<div class="multi-select-option ' + (data.chantier === c.idChantier ? 'selected' : '') + '" onclick="choisirChantier(' + c.idChantier + ')">' + escapeHtml(c.titre || 'Sans titre') + '</div>').join('');
    const courant = chantiers.find(c => c.idChantier === data.chantier);
    const choisi = courant
        ? '<span class="multi-select-chip">' + escapeHtml(courant.titre || 'Sans titre') + '<span class="remove" onclick="event.stopPropagation();choisirChantier(null)">×</span></span>'
        : '';
    return '<div class="panel-section"><div class="panel-section-header"><span class="panel-section-title">Chantier</span></div>' +
        '<div class="form-group">' + choisi +
        '<div class="multi-select" id="chantierSelect"><button class="addbtn" onclick="toggleMultiSelect(\'chantierSelect\')"><span style="font-size:1rem;line-height:1">+</span> ' +
        (courant ? 'Changer de chantier' : 'Choisir un chantier') + '</button>' +
        '<div class="multi-select-dropdown">' + recherche + options + sansResultat + '</div></div>' +
    '</div></div>';
}

function choisirChantier(id) {
    updateField('chantier', id || null);
    const menu = document.getElementById('chantierSelect');
    if (menu) menu.classList.remove('open');
}
...
function toggleFilterMenu(id) {
    ...
    const champ = m.querySelector('.fm-search');
    if (champ) { champ.value = ''; filtrerOptionsDeMenu(champ); champ.focus(); }
}

// Filtre les options sans redessiner le menu : la recherche en cours doit survivre au choix,
// sinon on ne peut pas en cocher deux d'affilee.
function filtrerOptionsDeMenu(champ) {
    const menu = champ.closest('.filter-menu');
    if (!menu) return;
    const q = sansAccents(champ.value.trim());
    let visibles = 0;
    menu.querySelectorAll('.filter-option[data-filtre]').forEach(opt => {
        const garde = !q || sansAccents(opt.textContent).indexOf(q) !== -1;
        opt.style.display = garde ? '' : 'none';
        if (garde) visibles++;
    });
    // Un separateur de domaine ne reste que s'il coiffe encore une ligne.
    menu.querySelectorAll('.fm-group-label').forEach(label => {
        let suivant = label.nextElementSibling, coiffe = false;
        while (suivant && !suivant.classList.contains('fm-group-label')) {
            if (suivant.classList.contains('filter-option') && suivant.style.display !== 'none') { coiffe = true; break; }
            suivant = suivant.nextElementSibling;
        }
        label.style.display = coiffe ? '' : 'none';
    });
    const vide = menu.querySelector('.fm-vide');
    if (vide) vide.style.display = visibles ? 'none' : '';
}

function toucheDansMenuDeFiltre(evenement, champ) {
    const menu = champ.closest('.filter-menu');
    if (evenement.key === 'Escape') { if (menu) menu.classList.remove('open'); return; }
    if (evenement.key !== 'Enter' || !menu) return;
    evenement.preventDefault();
    const premiere = Array.from(menu.querySelectorAll('.filter-option[data-filtre]'))
...
const parOrdreAlphabetique = (a, b) => String(a || '').localeCompare(String(b || ''), 'fr', { sensitivity: 'base' });
...
function optionsDeResponsables() {
    const membres = team.filter(m => m.actif !== false);
    if (!membres.length) return '<div class="filter-option muted">Aucun membre</div>';
    const groupes = domainesConnus().concat(membres.some(m => !m.Domaine) ? [SANS_DOMAINE] : []);
    return groupes.map((domaine, rang) => {
        const dedans = membres
            .filter(m => (m.Domaine || SANS_DOMAINE) === domaine)
            .sort((a, b) => parOrdreAlphabetique(a.nom, b.nom));
        if (!dedans.length) return '';
        return `<div class="fm-group-label${rang === 0 ? ' first' : ''}">${escapeHtml(domaine)}</div>`
            + dedans.map(m => optionDeFiltre('responsable', m.id, `toggleFilter('responsable', ${m.id})`,
                filters.responsable.includes(m.id), getTeamMemberColor(m.id), escapeHtml(m.nom))).join('');
    }).join('');
}
...
function updateFilterMenus() {
    ...
    const contenus = {
        ...
        responsable: optionsDeResponsables()
    };
    const recherche = '<div class="fm-search-zone"><input class="fm-search" type="text" placeholder="Rechercher"'
        + ' oninput="filtrerOptionsDeMenu(this)" onkeydown="toucheDansMenuDeFiltre(event, this)"></div>';
    for (const facette of FACETTES) {
        const menu = document.getElementById(facette.menu);
        if (menu) menu.innerHTML = recherche + contenus[facette.cle]
            + '<div class="fm-vide" style="display:none">Aucun résultat</div>'
            + `<div class="fm-foot"><span class="fm-clear" onclick="effacerFiltre('${facette.cle}')">Effacer</span></div>`;
    }
...
// Quadrillage, week-ends, colonne du jour et debuts de mois sont peints en fond de la grille et
// non en elements : une cellule par croisement ligne x colonne faisait plus de 800 000 noeuds sur
// le document du metier, reconstruits a chaque rendu.
function peindreFondGrille(grid, unite, start, totalDays, numCells, cw) {
    // Les couches se peignent de la premiere a la derniere : le quadrillage passe donc au-dessus
    // des week-ends et de la colonne du jour, comme la bordure des cellules le faisait.
    const couches = ['linear-gradient(to right, transparent 0 ' + (cw - 1) + 'px, #f1f5f9 ' + (cw - 1) + 'px ' + cw + 'px)'];
    const tailles = [cw + 'px 100%'];
    const positions = ['0 0'];
    const repetitions = ['repeat'];
    if (unite === 'day') {
        // Les week-ends reviennent tous les sept jours : une seule bande decalee sur le premier
        // samedi de la plage suffit a les peindre tous.
        couches.push('linear-gradient(to right, #fef2f2 0 ' + (2 * cw) + 'px, transparent ' + (2 * cw) + 'px ' + (7 * cw) + 'px)');
        tailles.push((7 * cw) + 'px 100%');
        positions.push((((6 - start.getDay() + 7) % 7) * cw) + 'px 0');
    ...
    let colonne;
    if (unite === 'month') colonne = (aujourdhui.getFullYear() - start.getFullYear()) * 12 + (aujourdhui.getMonth() - start.getMonth());
    else if (unite === 'week') colonne = Math.floor(getDaysDiff(start, aujourdhui) / 7);
    else colonne = getDaysDiff(start, aujourdhui);
    if (colonne >= 0 && colonne < numCells) {
        couches.push('linear-gradient(rgba(79, 70, 229, 0.08), rgba(79, 70, 229, 0.08))');
        tailles.push(cw + 'px 100%');
        positions.push((colonne * cw) + 'px 0');
        repetitions.push('no-repeat');
    }

    grid.style.backgroundImage = couches.join(', ');
    grid.style.backgroundSize = tailles.join(', ');
    grid.style.backgroundPosition = positions.join(', ');
    grid.style.backgroundRepeat = repetitions.join(', ');

    // Les mois n'ont pas tous la meme longueur : leurs separateurs restent des elements, un par
    // mois de la plage, la ou les cellules en posaient un par jour et par ligne.
    if (unite === 'day') {
        for (let i = 0; i < totalDays; i++) {
            if (addDays(start, i).getDate() !== 1) continue;
            const sep = document.createElement('div');
            sep.className = 'month-sep';
            sep.style.left = (i * cw) + 'px';
    ...
    if (todayOffset >= 0 && todayOffset <= totalDays) {
        ...
        if (!ganttNowAligned) {
            ...
            if (sc && sc.scrollWidth > sc.clientWidth) {
                ...
                // La plage recule jusqu'à la tâche la plus ancienne : acquérir le calage avant que
                // les tâches soient lues le fige sur une plage qui s'étend ensuite vers la gauche,
                // et le défilement retenu désigne alors la première année du document.
                ganttNowAligned = tachesLues;
            }
        }
    }
...
async function confirmClosePanel() {
    if (panelState.isNew && panelState.editData?.titre?.trim()) {
        const choix = await demanderChoix(
            'Fermer sans créer ?',
            'La fiche n’a pas encore été créée : ce qui a été saisi sera perdu.',
            [{ libelle: 'Continuer la saisie', valeur: null }, { libelle: 'Fermer sans créer', valeur: 'fermer', danger: true }]);
        if (choix === 'fermer') closePanel();
    } else { closePanel(); }
}
    ...
    content.innerHTML =
        ...
        groupeDeProps(
            ...
            '<div class="prop-row pr-status"><span class="prop-label">Statut</span><div class="prop-value"><div class="status-selector">' +
                (data.type !== 'jalon' ? statusCfg.list : statusCfg.list.filter(s => s.value === statusCfg.firstValue || s.value === statusCfg.terminalValue)).map(s => '<div class="status-pill ' + (data.statut === s.value ? 'selected' : '') + '" data-status="' + escapeAttr(s.value) + '" onclick="updateField(\'statut\', this.dataset.status)"><span class="panel-pill-dot" style="background:' + couleurSure(s.fillColor) + '"></span>' + escapeHtml(s.label) + '</div>').join('') +
            '</div></div></div>' +
            (data.type === 'tache' ?
        ...
        (data.type !== 'jalon' && !chantier ? sectionChantier(data, msSearch, msNoResult) : '') +
...
function updateField(field, value, noSave) {
    ...
    // sélection doit se voir dans la fiche sans attendre un rechargement. Le chantier aussi : il
    // décide de la branche où la tâche se range, et son absence d'ici laissait le volet afficher
    // l'ancien rattachement derrière le toast d'enregistrement.
    const visualFields = ['type', 'priorite', 'statut', 'projet', 'progression', 'dateDebut', 'dateEcheance', 'couleur', 'parentTask', 'Responsable', 'chantier'];
...
/**
 * Demande un choix dans une modale du widget. Un dialogue du navigateur s'ouvre au nom du domaine
 * qui sert le widget, au milieu d'une page qui n'est pas la sienne, et n'offre que deux réponses,
 * dont un « Annuler » auquel il fallait accrocher une action. Chaque bouton nomme ici son effet.
 * `choix` : [{ libelle, valeur, danger }]. Rend la valeur choisie, null si la modale est abandonnée.
 */
function demanderChoix(titre, message, choix) {
    return new Promise((resolve) => {
        const modale = document.getElementById('modaleChoix');
        modale.querySelector('.modale-titre').textContent = titre;
        modale.querySelector('.modale-texte').textContent = message;
        const actions = modale.querySelector('.modale-actions');
        actions.innerHTML = '';
        const fermer = (valeur) => {
        ...
        const surTouche = (e) => {
            ...
            e.stopPropagation();   // sans quoi la touche fermerait aussi le volet derrière
            fermer(null);
        };
        for (const c of choix) {
            const bouton = document.createElement('button');
            bouton.className = 'modale-btn' + (c.danger ? ' danger' : '');
            bouton.textContent = c.libelle;
            bouton.onclick = () => fermer(c.valeur);
            actions.appendChild(bouton);
        }
        modale.onclick = (e) => { if (e.target === modale) fermer(null); };
...
async function confirmDelete() {
    ...
    if (descendants.length) {
        const pluriel = descendants.length > 1;
        const choix = await demanderChoix(
            'Cette tâche contient ' + descendants.length + ' sous-tâche' + (pluriel ? 's' : ''),
            'Supprimer la tâche seule laisse ' + (pluriel ? 'ses sous-tâches' : 'sa sous-tâche') + ' dans le document, sans parent.',
            [
                { libelle: 'Annuler', valeur: null },
                { libelle: pluriel ? 'Détacher les sous-tâches' : 'Détacher la sous-tâche', valeur: 'detacher' },
                { libelle: 'Tout supprimer', valeur: 'cascade', danger: true }
            ]);
        if (!choix) { hideDeleteConfirm(); return; }
        if (choix === 'cascade') ids = [panelState.taskId, ...descendants.map(d => d.id)];
        else {
...
// Demander une table que le document ne porte pas fait journaliser une erreur par le bac à sable de
// Grist, avant même que notre garde ne la voie : une ligne rouge à chaque ouverture, dans la console
// où l'on cherche les vrais problèmes.
...
// Une table du document, ou null : absente du schéma elle n'est pas demandée, refusée elle est
// notée pour le bandeau. Jamais d'exception, l'appelant décide de ce qu'il fait du vide.
async function lireTable(nom) {
    if (!tableDeclaree(nom)) return null;
    try { return await grist.docApi.fetchTable(nom); } catch (e) { noterRefus(nom); return null; }
}

async function loadAllData(prefetched) {
    tablesRefusees = new Set();
    // Les cinq lectures partent ensemble : enchaînées, elles coûtaient autant d'allers-retours au
    // chargement comme à chaque retour sur le widget, pour des tables qui ne dépendent pas les unes
    // des autres. `prefetched` porte ce qu'ensureSchema a déjà lu à l'ouverture.
    const deja = prefetched || {};
    const [brutTasks, brutTeam, brutProjects, brutCategories, brutChantiers] = await Promise.all([
        deja.Tasks || lireTable('Tasks'),
        deja.Team || lireTable('Team'),
        deja.Projects || lireTable('Projects'),
        lireTable('Categorie_de_projet'),
        // Le rattachement se lit sur le type d'une colonne de Tasks : sans lui, la table des
        // chantiers n'a pas à être demandée.
        colonneChantier() ? lireTable('Chantiers') : null
    ]);
    TASK_COLS = new Set(Object.keys(brutTasks || {}));
    tasks = convert(brutTasks);
    team = convert(brutTeam);
    projects = convert(brutProjects);
    ...
    categoriesProjet = convert(brutCategories);
    gristReady = true;
    tachesLues = true;
    fusionnerChantiers(brutChantiers);
