...
const TF = (function () {
    ...
    function computeTimelineScale(opts) {
        ...
        let effectiveDays = Math.max(daysDiff(eStart, eEnd), viewDays);
        let numCells;
        if (unit === 'month') {
            ...
            // La grille pose une colonne par semaine : la plage doit donc couvrir des semaines
            // entieres. Sinon `pxPerDay` etale la plage sur la largeur des colonnes, sept jours ne
            // valent plus une colonne, et tout ce qui se place au jour (barres, jalons, ligne du
            // jour) glisse d'autant plus que la plage est longue.
            effectiveDays = numCells * 7;
        } else {
    ...
    function couleurDeDomaine(equipe, domaine) {
        const membre = (equipe || []).find(function (m) { return m.Domaine === domaine; });
        if (!membre) return '#6366f1';
        return membre.couleur || '#3e5de7';
    }

    return {
        DEFAULT_STATUSES: DEFAULT_STATUSES,
        couleurDeDomaine: couleurDeDomaine,
